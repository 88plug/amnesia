...
find_python() {
  ...
  for c in "${EIGHTYEIGHT_PYTHON:-}" "${PLUGIN_PYTHON:-}" "${AMNESIA_PYTHON:-}"; do
    _try "$c" && return 0
  done
  ...
  # Prefer python3 (PEP 394) before unversioned python.
  for c in python3 python3.13 python3.12 python3.11 python3.10 python; do
    _try "$c" && return 0
  done

  # Absolute fallbacks — critical for GUI/app spawns missing shell PATH.
  for c in \
...
PY="$(find_python)" || {
  echo "amnesia: no Python >=${MIN_MAJOR}.${MIN_MINOR} found." >&2
  echo "  Set EIGHTYEIGHT_PYTHON=/path/to/python3 (or AMNESIA_PYTHON), or install Python 3." >&2
